...
import { downloadNextExport, downloadNextTemplate, importNextExcel } from '../api/products';
...
const NextExcel = () => {
    ...
    const handleImport = async () => {
        ...
        setImportError(null);
        try {
            const res = await importNextExcel(importFile);
    ...
    return (
        <div className="p-6 max-w-4xl mx-auto">
            <div className="flex items-center gap-3 mb-8">
                ...
                <div>
                    <h1 className="text-2xl font-bold text-slate-900 dark:text-white">{t(tx('Excel (Next)', 'إكسيل (نكست)'))}</h1>
                    <p className="text-slate-600 dark:text-slate-400 text-sm">{t(tx('Export, template, and import Next products from Excel.', 'تصدير وتنزيل القالب واستيراد منتجات نكست من Excel.'))}</p>
                </div>
            </div>
            ...
            <div className="grid gap-6 md:grid-cols-1">
                <div className={blockClass}>
                    ...
                    <p className="text-sm text-slate-600 dark:text-slate-400 mb-4">{t(tx('Download all Next products as an Excel file (one row per variant).', 'تنزيل كل منتجات نكست كملف Excel (صف لكل متغير).'))}</p>
                    <button
                        type="button"
                ...
                <div className={blockClass}>
                    ...
                    <p className="text-sm text-slate-600 dark:text-slate-400 mb-4">{t(tx('Upload an Excel file (.xlsx or .xls) to create Next products. Use the template for the correct columns.', 'ارفع ملف Excel (.xlsx أو .xls) لإنشاء منتجات نكست. استخدم القالب للأعمدة الصحيحة.'))}</p>
